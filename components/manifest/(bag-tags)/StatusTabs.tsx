// app/dashboard/manifest/bag-tags/components/StatusTabs.tsx
import { Card, CardContent } from "@/components/ui/card";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { Package, CheckCircle2, Truck, Lock, XCircle } from "lucide-react";
...
const StatusTabs = ({ activeTab, setActiveTab, data }: StatusTabsProps) => {
  const getStatusCount = (status: string) => {
    return data.filter((bag) => status === "all" || bag.status === status)
      .length;
  };
